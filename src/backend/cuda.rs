//! CUDA backend implementation using llama.cpp with GPU offload.
//!
//! 通过 LlamaHardware trait 抽象硬件差异，复用 LlamaRunner 公共推理逻辑。
...
impl Default for CudaHardware {
    fn default() -> Self {
        Self::new()
    }
}

impl LlamaHardware for CudaHardware {
...
/// CudaBackend - CUDA 后端实现
pub struct CudaBackend {
    runner: LlamaRunner<CudaHardware>,
}

impl CudaBackend {
    pub fn new<P: AsRef<Path>>(model_path: P, n_ctx: i32) -> InferenceResult<Self> {
        let mut runner = LlamaRunner::new(CudaHardware::new());
...
#[async_trait]
impl BaseBackend for CudaBackend {
...
#[async_trait]
impl LLMBackend for CudaBackend {
...
impl InferenceBackend for CudaBackend {
    ...
    fn forward(&self, tokens: &[i32], config: &InferenceConfig) -> InferenceResult<ForwardResult> {
        ...
        while let Some(chunk) = rt.block_on(receiver.into_stream().next()) {
            match chunk {
                StreamChunk::Normal(crate::backend::traits::LLMSteamChunkType::RawToken(text)) => {
