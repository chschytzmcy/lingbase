...
impl InferenceBackend for CudaBackend {
    ...
    fn forward(&self, tokens: &[i32], config: &InferenceConfig) -> InferenceResult<ForwardResult> {
        ...
        let ctx = ctx_guard.as_ref().ok_or(InferenceError::BackendNotInitialized)?;

        let mut batch = BatchWithTokens::new(tokens)?;
        ctx.decode(batch.batch)?;

        let logits = ctx.get_logits(self.n_vocab).ok_or(InferenceError::InferenceFailed(
    ...
    fn forward_stream(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            if ctx.is_none() {
                let _ = tx.blocking_send(Err(InferenceError::BackendNotInitialized));
                return;
            }
            let ctx = ctx.unwrap();
            ...
            let logits = match ctx.get_logits(n_vocab) {
                Some(l) => l,
                None => {
                    let _ = tx.blocking_send(Err(InferenceError::InferenceFailed(
                        "Failed to get logits".to_string()
                    )));
                    return;
                }
    ...
    fn tokenize(&self, text: &str) -> InferenceResult<Vec<i32>> {
        ...
        let tokenizer = tokenizer_guard.as_ref()
            .ok_or(InferenceError::BackendNotInitialized)?;
